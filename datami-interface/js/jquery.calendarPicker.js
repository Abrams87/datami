jQuery.fn.calendarPicker = function(options) {
  ...
  calendar.changeDate = function(date) {
    ...
    var fillDays = function(date) {
      ...
      for (var i = -options.days; i <= options.days; i++) {
        var d = new Date(date);
        d.setDate(day + i)
	    var span = $("<span>").addClass("calElement").addClass("calDay").attr("millis", d.getTime())
        if (i == -options.days && options.showDayArrows) {
          ...
          if (d.getYear() == calendar.currentDate.getYear() && d.getMonth() == calendar.currentDate.getMonth() && d.getDate() == calendar.currentDate.getDate())
            span.addClass("selected");
	  if ((d >= calendar.startDate && d <= calendar.endDate) || (d >= calendar.endDate && d <= calendar.startDate))
	      span.addClass("datamiselected");
        }
        divDays.append(span);
  ...
  // this is what happens if we click on a date...
  theDiv.click(function(ev) {
	  var el = $(ev.target).closest(".calElement");
	  if (el.hasClass("calElement")) {
	      date = new Date(parseInt(el.attr("millis")));
	      if (el.hasClass("calDay") && !el.hasClass("next") && !el.hasClass("prev")){
		  if (calendar.lastSelected==-1){
		      calendar.startDate = date;
		      calendar.endDate = date;
		      calendar.lastSelected = 0;
		  } else {
		      if (calendar.lastSelected==0){	       
			  calendar.endDate = date;
			  calendar.lastSelected=1;
		      } else {
			  calendar.startDate = date;
